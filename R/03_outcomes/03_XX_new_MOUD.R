...
# MET
moud_cohort <- left_join(cohort, met, by="BENE_ID")
...
moud_cohort <- moud_cohort |>
  ...
  slice(1) |>
  ungroup() |>
  select(BENE_ID, has_new_met = has_new_moud, moud_start_dt)

saveRDS(moud_cohort, "/mnt/general-data/disability/post_surgery_opioid_use/outcomes/cohort_has_new_met.rds")


# NAL
moud_cohort <- left_join(cohort, nal, by="BENE_ID")
...
moud_cohort <- moud_cohort |>
  ...
  slice(1) |>
  ungroup() |>
  select(BENE_ID, has_new_nal = has_new_moud, moud_start_dt)

saveRDS(moud_cohort, "/mnt/general-data/disability/post_surgery_opioid_use/outcomes/cohort_has_new_nal.rds")

# BUP

moud_cohort <- left_join(cohort, bup, by="BENE_ID")
...
moud_cohort <- moud_cohort |>
  ...
  slice(1) |>
  ungroup() |>
  select(BENE_ID, has_new_bup = has_new_moud, moud_start_dt)

saveRDS(moud_cohort, "/mnt/general-data/disability/post_surgery_opioid_use/outcomes/cohort_has_new_bup.rds")