# -------------------------------------
# Script: 04_02_confound_depression
...
depression_icds <- read_csv("~/medicaid/post_surgery_opioid_use/R/04_confounders/depression_icd10_20230323.csv", col_names = F) |>
...
# saveRDS(all_depression, "/mnt/general-data/disability/post_surgery_opioid_use/confounders/confounder_depression.rds") # save final data file

cohort <- merge(cohort, all_depression, by="BENE_ID")

cohort <- cohort |>
  mutate(has_depression = as.numeric(depression_dt < followup_start_dt)) |>
  select(BENE_ID, has_depression)

saveRDS(cohort, "/mnt/general-data/disability/post_surgery_opioid_use/confounders/confounder_depression.rds")