...
washout <- readRDS("/mnt/general-data/disability/post_surgery_opioid_use/intermediate/first_surgeries.rds") |>
  mutate(study_end_dt = discharge_dt %m+% months(24)) |>
  select(-PRCDR_CD) |>
  as.data.table()
...
dates <- unique(dates[
  ENRLMT_START_DT <= study_end_dt & 
    ENRLMT_END_DT >= discharge_dt
][
  , `:=`(
    # “floor” the start date at first_treatment_dt
    ENRLMT_START_DT = fifelse(ENRLMT_START_DT < discharge_dt, discharge_dt, ENRLMT_START_DT),
...
for (i in seq_along(chunks)) {
  file_name <- paste0(drv_root,
                      "/sensitivity_analysis/outcomes/tmp_post_exposure/enrollment_period_chunk_", 
                      sprintf("%02d", i), ".rds"
  )
