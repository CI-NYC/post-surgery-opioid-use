...
for (i in c("")){
  ...
  cohort <- fmutate(
    cohort, 
    period_1 = interval(
      discharge_dt, discharge_dt %m+% months(6) # exposure period + period 1
    ), 
    period_2 = interval(
  ...
  dual_cens <- 
    ...
    filter(elig_dt %within% interval(discharge_dt, discharge_dt + months(24))) |> 
    group_by(BENE_ID) |>
    arrange(elig_dt) |> 
    filter(row_number() == 1) |> 
    ungroup() |> 
  ...
  dual_codes2 <- 
    ...
    fselect(BENE_ID, washout_start_dt, discharge_dt, code, elig_dt) |> 
  ...
  enrollment_cens <- load_data(paste0("all_possible_enrollment_dates/censoring_enrollment", i, ".fst"), file.path(drv_root, "sensitivity_analysis/outcomes")) |>
  ...
  write_data(cens, paste0("pain_washout_continuous_enrollment_censoring", i, ".fst"), file.path(drv_root, "sensitivity_analysis/outcomes"))
}