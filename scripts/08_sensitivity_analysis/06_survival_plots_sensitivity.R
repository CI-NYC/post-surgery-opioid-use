...
load_dir <- "/mnt/general-data/disability/post_surgery_opioid_use/sensitivity_analysis/analysis/earth"
result_dir <- "/mnt/general-data/disability/post_surgery_opioid_use/sensitivity_analysis/analysis/earth/plots"
...
plot_surv <- function(outcome, c_section_identifier){
  ...
  p <- ggplot(combined_results, aes(x = t, y = estimate)) + 
    ...
    geom_errorbar(aes(ymin = conf.low, ymax = conf.high, colour = Intervention), width = 0.2) + 
    labs(x = "", y = "Absolute risk", title="(A)") + 
    scale_x_continuous(labels = c("6","12","18","24")) +
...
combine_contrasts <- function(outcome, shift, c_section_identifier){
  ...
  for (i in 1:4){
    results_contrast <- rbind(results_contrast, lmtp_object$contrast[[i]]$estimates)
  }
...
p1 <- plot_surv("oud_hillary_period", "other")
p2 <- plot_survdiff("oud_hillary_period", "other")
pdf(file.path(result_dir, "plots_other_hillary.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("oud_period", "other")
p2 <- plot_survdiff("oud_period", "other")
pdf(file.path(result_dir, "plots_other_OUD.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("moud_period", "other")
p2 <- plot_survdiff("moud_period", "other")
...
p1 <- plot_surv("oud_hillary_period", "c-section")
p2 <- plot_survdiff("oud_hillary_period", "c-section")
pdf(file.path(result_dir, "plots_c-section_hillary.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("oud_period", "c-section")
p2 <- plot_survdiff("oud_period", "c-section")
pdf(file.path(result_dir, "plots_c-section_OUD.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("moud_period", "c-section")
p2 <- plot_survdiff("moud_period", "c-section")
