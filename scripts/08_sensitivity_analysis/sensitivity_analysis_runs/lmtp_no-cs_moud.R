...
version <- "earth"
Ys <- c("moud_period")
Cs <- c("cens_moud_period_")
print(paste0("CF_folds: ", CF_folds, ", Version: ", version, ", ", paste(Ys)))

dat_non_c_section <- load_data("df_non_c_section.fst", file.path(drv_root, "sensitivity_analysis/final")) |>
  as.data.frame()

dat_only_c_section <- load_data("df_only_c_section.fst", file.path(drv_root, "sensitivity_analysis/final")) |>
  as.data.frame()
...
lmtp_contrast_and_save <- function(Y, W, data, shifted, shift_name, results_observ, c_section_identifier){
  ...
  saveRDS(results, file.path("/mnt/general-data/disability/post_surgery_opioid_use/sensitivity_analysis/analysis", version,
...
run_lmtp <- function(dat_lmtp, W, c_section_identifier) {
  ...
  for (Y in Ys){
    # results_observ <- readRDS(file.path("/mnt/general-data/disability/post_surgery_opioid_use/sensitivity_analysis/analysis", version,
    #                                     paste0("lmtp_result_", c_section_identifier, "_", "shift_1", "_", Y, ".rds")))$observed
