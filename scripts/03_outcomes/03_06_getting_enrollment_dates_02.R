...
files <- file.path(drv_root, "outcomes/tmp_post_exposure") |>
...
for (i in seq_along(files)){
  print(i)
  final_df <- load_data(paste0("outcomes/all_possible_enrollment_dates/combined_all_enrolled_dates_cohort_", sprintf("%02d", i), ".fst"), drv_root)
...
write_data(
  combined_all_df,
  paste0("all_possible_enrollment_dates/combined_all_enrolled_dates_cohort_ALL", ".fst"), file.path(drv_root, "outcomes")
)
