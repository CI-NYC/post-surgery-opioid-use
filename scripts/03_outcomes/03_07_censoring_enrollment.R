...
all_enrollment_dates <- load_data("all_possible_enrollment_dates/combined_all_enrolled_dates_cohort_ALL.fst", file.path(drv_root, "outcomes")) |>
  left_join(cohort) |>
  # left_join(cohort_7_day_gap) |>
  filter(date >= followup_start_dt) # only need dates from post-exposure start


# creating exposure end date periods (each period is 91 days long)

all_enrollment_dates <- all_enrollment_dates |>
  mutate(period_1_end_dt = followup_start_dt %m+% months(6),
...
all_enrollment_dates <- all_enrollment_dates |>
  mutate(enrolled_during_period_1 = case_when(date >= followup_start_dt + days(0) & date <= period_1_end_dt & enrolled == 1 ~ 1,
                                              TRUE ~ 0),
         ...
         enrolled_during_period_4 = case_when(date >= period_3_end_dt + days(0) & date <= period_4_end_dt & enrolled == 1 ~ 1, # if they were enrolled on the LAST day of the previous period, their censoring should start in this period
                                              TRUE ~ 0),
         study_end_dt = followup_start_dt + months(24),
...
write_data(all_enrollment_dates_grouped, "all_possible_enrollment_dates/censoring_enrollment.fst", file.path(drv_root, "outcomes"))
