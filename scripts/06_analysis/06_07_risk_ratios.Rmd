...
```{r}
cs_dat <- read.csv("/mnt/general-data/disability/post_surgery_opioid_use/analysis/earth/plots/lmtp_results_c-section_oud_hillary_period.csv") |>
...
cs_dat <- cs_dat |> mutate(t = rep(c(1,2,3,4), 3)) |> arrange(shift, t)
...
contrast_rr_single <- function(fit, ref) {
  ...
  # Helper: reconstruct EIF for risk
  get_risk_eif <- function(obj, y, scale) {
    Q <- obj$outcome_reg
    H <- obj$density_ratios
    est <- obj$estimate
    phi_S <- H * (y - Q) + (Q - est)
    -phi_S
  }

  # point estimates on risk scale
  risk_fit <- 1 - fit$estimate
  risk_ref <- 1 - ref$estimate
  theta    <- risk_fit / risk_ref

  # per-observation EIFs for risk
  eif_risk_fit <- get_risk_eif(fit, y, scale)
  eif_risk_ref <- get_risk_eif(ref, y, scale)

  # EIF for log risk ratio
  log_eif <- eif_risk_fit / risk_fit - eif_risk_ref / risk_ref

  # Cluster IDs
  if (is.null(fit$id)) fit$id <- seq_along(log_eif)

  clusters <- split(log_eif, fit$id)
  j        <- length(clusters)
  std.error <- sqrt(var(vapply(clusters, mean, 1)) / j)

  # CI + p-value
  log_theta <- log(theta)
  zcrit     <- qnorm(0.975)
  conf.low  <- exp(log_theta - zcrit * std.error)
  conf.high <- exp(log_theta + zcrit * std.error)
  p.value   <- 2 * pnorm(abs(log_theta) / std.error, lower.tail = FALSE)
...
result <- read.csv("/mnt/general-data/disability/post_surgery_opioid_use/analysis/xgboost/plots/lmtp_contrasts_c-section_oud_hillary_period.csv")
...
```{r}
no_cs_dat <- read.csv("/mnt/general-data/disability/post_surgery_opioid_use/analysis/earth/plots/lmtp_results_other_oud_hillary_period.csv") |>
  select(estimate, std.error, conf.low, conf.high, Intervention, shift) |>
  as.data.table()

no_cs_dat <- cbind(no_cs_dat[Intervention == "hypothetical reduction"], no_cs_dat[Intervention == "no reduction"])[, -c(5, 6, 11)]

colnames(no_cs_dat) <- c("estimate.i", "std.error.i", "conf.low.i", "conf.high.i", "estimate.o", "std.error.o", "conf.low.o", "conf.high.o", "shift")

no_cs_dat <- no_cs_dat |> mutate(t = rep(c(1,2,3,4), 3)) |> arrange(shift, t)
...
for (shift in 1:3){
  results <- readRDS(paste0("/mnt/general-data/disability/post_surgery_opioid_use/analysis/xgboost/lmtp_result_other_shift_",
                            shift, "_Y2.rds"))
  for (t in 1:4) {
    if (t == 0){
      result_df <- rbind(result_df, c(0,0,0,0,0,0,0))
...
result <- read.csv("/mnt/general-data/disability/post_surgery_opioid_use/analysis/xgboost/plots/lmtp_contrasts_other_oud_hillary_period.csv")
result_df <- cbind(result_df, result |> arrange(shift,t) |>
                     select(RD = theta, RD.CI.low = conf.low, RD.CI.high = conf.high)) |>
    mutate(across(3:5, ~ round(., 3)),
           across(6:8, ~ round(., 6)))
