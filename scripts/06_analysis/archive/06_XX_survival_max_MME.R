...
dat_non_c_section <- readRDS("/mnt/general-data/disability/post_surgery_opioid_use/final/df_non_c_section_max.rds") |>
  as.data.frame()

dat_only_c_section <- readRDS("/mnt/general-data/disability/post_surgery_opioid_use/final/df_only_c_section_max.rds") |>
  as.data.frame()
...
A <- list(c("max_daily_dose_mme", "days_supplied")) # removed days_supplied
...
lmtp_contrast_and_save <- function(Y, W, data, shifted, shift_name, results_observ, c_section_identifier){
  ...
  #                            paste0("lmtp_result_", c_section_identifier, "_", shift_name, "_", Y, "max.rds")))
}
...
run_lmtp <- function(dat_lmtp, W, c_section_identifier) {
  shift_1 <- shift_data(dat_lmtp, c("max_daily_dose_mme", "days_supplied"))
  shift_2 <- shift_data(dat_lmtp, c("max_daily_dose_mme"))
