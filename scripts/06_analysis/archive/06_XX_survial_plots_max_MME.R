...
combine_intervention_observed <- function(outcome, shift, c_section_identifier) {
  ...
  # load lmtp results
  lmtp_object <- readRDS(file.path(load_dir, paste0("lmtp_result_", c_section_identifier, "_", shift, "_", outcome,"max.rds")))
...
plot_surv <- function(outcome, c_section_identifier){
  ...
  write.csv(combined_results, paste0(result_dir, "/lmtp_results_", c_section_identifier, "_", outcome, "max.csv"))
...
plot_survdiff <- function(outcome, c_section_identifier){
  ...
  # write.csv(results_contrast, paste0(result_dir, "/lmtp_contrasts_", c_section_identifier, "_", outcome, "max.csv"))
...
pdf(file.path(result_dir, "maxplots_other_hillary.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("Y3", "other")
p2 <- plot_survdiff("Y3", "other")
pdf(file.path(result_dir, "maxplots_other_OUD.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("Y4", "other")
p2 <- plot_survdiff("Y4", "other")
pdf(file.path(result_dir, "maxplots_other_MOUD.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol = 1)
dev.off()
...
pdf(file.path(result_dir, "maxplots_c-section_hillary.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("Y3", "c-section")
p2 <- plot_survdiff("Y3", "c-section")
pdf(file.path(result_dir, "maxplots_c-section_OUD.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()

p1 <- plot_surv("Y4", "c-section")
p2 <- plot_survdiff("Y4", "c-section")
pdf(file.path(result_dir, "maxplots_c-section_MOUD.pdf"), width = 9, height = 6)
grid.arrange(p1, p2, ncol= 1)
dev.off()